...
import api from '@/services/api/activities';
import {
  buildSuccess,
  handleError,
...
const module = {
  ...
  mutations: {
    ...
    update(state, { id, data }) {
      const indexToUpdate = state.activities.findIndex(
        (member) => member.id === id,
      );
      state.activities.splice(indexToUpdate, 1, {
        ...data,
      });
    },
    delete(state, id) {
      const indexToDelete = state.activities.findIndex(
        (member) => member.id === id,
      );
  ...
  getters: {
    getBrandNameById: (state) => (id) => {
      const brandFound = state.activities.find((brand) => brand.id === id);
