...
const model = require('../models/OverviewActivities');
...
const countByPlayer = async (req, res) => {
  try {
    ...
    let data = await model.aggregate(aggregatorOpts).exec();
    res.status(200).json({ ok: true, payload: data });
  } catch (error) {
    utils.handleError(res, error);
  }
};
